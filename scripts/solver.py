...
def calcul_contraintes(section, torseur, param_gene, type_profile, facteurs):       
        ...
        if bool(param_gene['trac_res']) is True:
                ax = stress_post.plot_stress(stress="n_zz", nrows=3, ncols=3, figsize=(15, 10), render=False, title="Traction liée à l'effort normal")
                fig = ax.get_figure()
                stress_post.plot_stress(stress="mxx_zz", ax=fig.axes[1], title="Flexion liée au moment suivant X")
                stress_post.plot_stress(stress="myy_zz", ax=fig.axes[2], title="Flexion liée au moment suivant Y")
                stress_post.plot_stress(stress="vx_zxy", ax=fig.axes[3], title="Cisaillement lié à l'effort suivant X")
                stress_post.plot_stress(stress="vy_zxy", ax=fig.axes[4], title="Cisaillement lié à l'effort suivant Y")
                stress_post.plot_stress(stress="mzz_zxy", ax=fig.axes[5], title="Torsion liée au moment suivant Z")
                stress_post.plot_stress(stress="zz", ax=fig.axes[6], title="Traction tous chargements")
                stress_post.plot_stress(stress="zxy", ax=fig.axes[7], title="Cisaillement tous chargements")
